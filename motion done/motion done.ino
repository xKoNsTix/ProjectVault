...
void loop() {
  ...
  if (distance < MIN_DISTANCE || distance > NOTHING_DISTANCE) {
    ...
    // if the distance is within the specified range (0-n cm),
    // map the distance to a servo position (0-90 degrees) and write it to the servos
    ...
    // if the distance is greater than 30 cm, move the servos to the 0 position
    servo1.write(0);
    servo2.write(0);
    delay(200);
  }
